...
lstm_att_router = APIRouter(prefix="/lstm_att", tags=["lstm_att"])
...
with open(VOCAB_PATH / "lstm_attention.json") as f:
    vocab = json.load(f)
...
model = LSTM_Attention_HallucinationDetector(embedding_dim, hidden_dim, vocab_size)
model.load_state_dict(torch.load(MODEL_SAVE_PATH / "lstm_attention/model.pt", map_location=device))
model.to(device)
model.eval()

@lstm_att_router.post("/predict")
